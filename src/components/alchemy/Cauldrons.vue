...
=======
        Track your alchemy progress of bubbles and see the costs/bonuses of
        those upgrades.
>>>>>>> 8fcf80e... fix: build errors for 0.3.0
      </p>
    </div>
  </div>
  <ul
    ...
    role="tablist"
    :style="{ background: ColorData[activeGroup].hex }"
  >
  ...
  <div class="d-flex flex-column alchemy-wrapper mt-3">
    <table class="border-top border-bottom text-light">
      ...
      <tr :style="{ display: 'flex', justifyContent: 'space-between' }">
        ...
        <th
          class="quarter centered whitespace-pre"
    ...
    <table class="text-light">
      ...
      <tr class="alter-color" v-for="n in 15" :key="n">
        <AlchemyRow
          @customChange="handleDiscountRecalc()"
          :group="activeGroup"
          :idx="amountBubbles - n"
          :discount="discount"
        />
...
import { AlchemyColor } from "~/composables/Alchemy";
...
const ColorData: Record<AlchemyColor, CauldronInfo> = {
  Orange: {
    name: "Power",
    hex: "#ff9420",
  },
...
//       let twelveBubbleLevel =
//         alchemy.value.upgrades[activeGroup.value][BARGAIN_BUBBLE];
//       let undevLevel =
//         alchemy.value.upgrades[UNDEV_COST_BUBBLE.color][
//           UNDEV_COST_BUBBLE.number
//         ];
